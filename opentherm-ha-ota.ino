...
void updateData()
{
  ...
  client.publish(STATE_DHW_GET_TOPIC.c_str(), enableHotWater ? "heat" : "off");
  ...
  client.publish(HIGH_TEMP_GET_TOPIC.c_str(), String(ophi).c_str());
  client.publish(LOW_TEMP_GET_TOPIC.c_str(), String(oplo).c_str());
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  else if (topicStr == HIGH_TEMP_SET_TOPIC) {
    float ophi1 = payloadStr.toFloat();
    if (isnan(ophi1) || !isValidNumber(payloadStr)) {
      Serial.println("Current high temp set is not a valid number, ignoring...");
    }
    else {
      ophi = ophi1;
    }
  }
  else if (topicStr == LOW_TEMP_SET_TOPIC) {
    float oplo1 = payloadStr.toFloat();
    if (isnan(oplo1) || !isValidNumber(payloadStr)) {
      Serial.println("Current high temp set is not a valid number, ignoring...");
    }
    else {
      oplo = oplo1;
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId, mqtt_user, mqtt_password)) {
      ...
      client.subscribe(HIGH_TEMP_SET_TOPIC.c_str());
      client.subscribe(LOW_TEMP_SET_TOPIC.c_str());
    } else {
...
void setup()
{
  ...
  ArduinoOTA.setHostname("Opentherm"); // give an name to our module
